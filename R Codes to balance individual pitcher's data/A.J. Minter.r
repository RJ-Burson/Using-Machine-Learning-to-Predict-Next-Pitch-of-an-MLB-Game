...
train_set= subset(dfvnew_train, dfvnew_train$pitcher=="A.J. Minter")[,-c(1,3)]
validation_set= subset(dfvnew_valid, dfvnew_valid$pitcher=="A.J. Minter")[,-c(1,3)]
final_test= subset(dfvnew_test, dfvnew_test$pitcher=="A.J. Minter")[,-c(1,3)]
...
BalancedData1 = ovun.sample(pitch_name~. , cut_ff, method="over", p=0.66, subset=options("subset")$subset, 
                            na.action=options("na.action")$na.action, seed)$data

BalancedData2 = ovun.sample(pitch_name~. , cut_ch, method="over", p=0.66, subset=options("subset")$subset, 
                            na.action=options("na.action")$na.action, seed)$data
