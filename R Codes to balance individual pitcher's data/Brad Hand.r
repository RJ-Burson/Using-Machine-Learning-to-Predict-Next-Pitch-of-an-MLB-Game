...
train_set= subset(dfvnew_train, dfvnew_train$pitcher=="Brad Hand")[,-c(1,3)]
validation_set= subset(dfvnew_valid, dfvnew_valid$pitcher=="Brad Hand")[,-c(1,3)]
final_test= subset(dfvnew_test, dfvnew_test$pitcher=="Brad Hand")[,-c(1,3)]


swp_ff= subset(train_set, pitch_name==8 | pitch_name==1)
swp_si= subset(train_set, pitch_name==8 | pitch_name==5)



BalancedData1 = ovun.sample(pitch_name~. , swp_ff, method="over", p=0.66, subset=options("subset")$subset, 
                            na.action=options("na.action")$na.action, seed)$data

BalancedData2 = ovun.sample(pitch_name~. , swp_si, method="over", p=0.66, subset=options("subset")$subset,
                            na.action=options("na.action")$na.action, seed)$data
