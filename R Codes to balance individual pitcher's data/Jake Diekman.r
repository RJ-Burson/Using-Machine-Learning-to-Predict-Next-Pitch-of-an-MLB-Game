...
train_set= subset(dfvnew_train, dfvnew_train$pitcher=="Jake Diekman")[,-c(1,3)]
validation_set= subset(dfvnew_valid, dfvnew_valid$pitcher=="Jake Diekman")[,-c(1,3)]
final_test= subset(dfvnew_test, dfvnew_test$pitcher=="Jake Diekman")[,-c(1,3)]


ff_ch= subset(train_set, pitch_name==1 | pitch_name==2)
ff_si= subset(train_set, pitch_name==1 | pitch_name==5)
ff_swp= subset(train_set, pitch_name==1 | pitch_name==8)
...
BalancedData3 = ovun.sample(pitch_name~. , ff_swp, method="over", p=0.75, subset=options("subset")$subset, 
                            na.action=options("na.action")$na.action, seed)$data
