...
train_set= subset(dfvnew_train, dfvnew_train$pitcher=="Alex Wood")[,-c(1,3)]
validation_set= subset(dfvnew_valid, dfvnew_valid$pitcher=="Alex Wood")[,-c(1,3)]
final_test= subset(dfvnew_test, dfvnew_test$pitcher=="Alex Wood")[,-c(1,3)]
...
BalancedData1 = ovun.sample(pitch_name~. , si_ch, method="over", p=0.666, subset=options("subset")$subset, 
                            na.action=options("na.action")$na.action, seed)$data

BalancedData2 = ovun.sample(pitch_name~. , si_sl, method="over", p=0.666, subset=options("subset")$subset,
                            na.action=options("na.action")$na.action, seed)$data
