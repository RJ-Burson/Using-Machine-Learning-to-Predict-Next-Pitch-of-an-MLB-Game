...
train_set= subset(dfvnew_train, dfvnew_train$pitcher=="Eduardo Rodriguez")[,-c(1,3)]
validation_set= subset(dfvnew_valid, dfvnew_valid$pitcher=="Eduardo Rodriguez")[,-c(1,3)]
final_test= subset(dfvnew_test, dfvnew_test$pitcher=="Eduardo Rodriguez")[,-c(1,3)]
...
BalancedData3 = ovun.sample(pitch_name~. , ff_si, method="over", p=0.8, subset=options("subset")$subset,
                            na.action=options("na.action")$na.action, seed)$data

BalancedData4 = ovun.sample(pitch_name~. , ff_sl, method="over", p=0.8, subset=options("subset")$subset, 
                            na.action=options("na.action")$na.action, seed)$data
