...
train_set= subset(dfvnew_train, dfvnew_train$pitcher=="Hirokazu Sawamura")[,-c(1,3)]
validation_set= subset(dfvnew_valid, dfvnew_valid$pitcher=="Hirokazu Sawamura")[,-c(1,3)]
final_test= subset(dfvnew_test, dfvnew_test$pitcher=="Hirokazu Sawamura")[,-c(1,3)]
...
BalancedData1 = ovun.sample(pitch_name~. , ff_sl, method="over", p=0.66, subset=options("subset")$subset,
                            na.action=options("na.action")$na.action, seed)$data

BalancedData2 = ovun.sample(pitch_name~. , ff_spl, method="over", p=0.66, subset=options("subset")$subset,
                            na.action=options("na.action")$na.action, seed)$data
