...
train_set= subset(dfvnew_train, dfvnew_train$pitcher=="Frankie Montas")[,-c(1,3)]
validation_set= subset(dfvnew_valid, dfvnew_valid$pitcher=="Frankie Montas")[,-c(1,3)]
final_test= subset(dfvnew_test, dfvnew_test$pitcher=="Frankie Montas")[,-c(1,3)]
...
BalancedData2 = ovun.sample(pitch_name~. , si_cut, method="over", p=0.8, subset=options("subset")$subset,
                            na.action=options("na.action")$na.action, seed)$data

BalancedData3 = ovun.sample(pitch_name~. , si_sl, method="over", p=0.8, subset=options("subset")$subset, 
                            na.action=options("na.action")$na.action, seed)$data

BalancedData4 = ovun.sample(pitch_name~. , si_spl, method="over", p=0.8, subset=options("subset")$subset,
                            na.action=options("na.action")$na.action, seed)$data
