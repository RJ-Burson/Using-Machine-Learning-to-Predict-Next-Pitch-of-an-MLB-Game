...
# Replacing all variables that have players id #s into the players names by matching players 
# full names in the 2021 pitches dataset and the mlbids dataset
...
# Replacing the NAs with actual players scaled stats. 
#Do so by matching players names from the ids dataset and the catcher blocking/throwing, stolen bases, and OPS datasets
...
# Replacing remaining NA values in the OPS and catcher variables with the mean values of each respective variable. 
# The existing NAs still remain because some catchers didnt play enough to qualify for some the the statistics. 
#So they arent in the OPS or catching datasets and no way to match their names.  
...
# Replacing all variables that have players id #s into the players names by matching players full names 
# in the 2021 pitches dataset and the mlbids dataset
df[["batter"]] = ids[ match(df[["batter"]], ids[["key_mlbam"]]), 'Full_Name']
df[["pitcher"]] = ids[ match(df[["pitcher"]], ids[["key_mlbam"]]), 'Full_Name']
df[["fielder_2"]] = ids[ match(df[["fielder_2"]], ids[["key_mlbam"]]), 'Full_Name']
df[["on_3b"]] = ids[ match(df[["on_3b"]], ids[["key_mlbam"]]), 'Full_Name']
...
# Replacing all variables that have players id #s into the players names by matching 
# players full names in the 2021 pitches dataset and the mlbids dataset
df[["batter"]] = ids[ match(df[["batter"]], ids[["key_mlbam"]]), 'Full_Name']
df[["pitcher"]] = ids[ match(df[["pitcher"]], ids[["key_mlbam"]]), 'Full_Name']
df[["fielder_2"]] = ids[ match(df[["fielder_2"]], ids[["key_mlbam"]]), 'Full_Name']
df[["on_3b"]] = ids[ match(df[["on_3b"]], ids[["key_mlbam"]]), 'Full_Name']
...
# Did it this complicated way because there are pitchers who threw 500 pitches in both seasons 
#but are not in the validation or test_set because they got injured or came back from an injury 
#halfway through the season. Walker Buehler is one example who got injured halfway through the 2022 season.
...
# Creating the new training, validation, and test sets with only the pitchers that threw 500 pitches in both
#the 2021 and 2022 seasons and were in all3 datasets (2021 season, pre all-star break 2022, and post all-star break 2022)
...
# Code to change factor variable levels from strings into integers for the 
# variables pitch name, previous pitch, and the previous pitch result
